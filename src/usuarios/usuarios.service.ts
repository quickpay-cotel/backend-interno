...
export class UsuariosService {
  ...
  async updateToken(usuario_id: number, token: string) {
    let user = await this.usuarioUsuariosRepository.updateToken(
      usuario_id,
      token,
    );
    return user;
  ...
  async setPassword(userName: string, plainPassword: string): Promise<void> {
    ...
    await this.usuarioUsuariosRepository.setPassword(
      usuario.usuario_id,
      hashedPassword,
    );
  }
  async getOptions(userName: string) {
    try {
      let menus =
        await this.usuarioUsuariosRepository.findOptionsByUserName(userName);
  ...
  async obtenerDatosPersonaEmpresa(usuarioId: number) {
    try {
      let datosPersonaEmpresa =
        await this.usuarioUsuariosRepository.findDatosPersonaAndEmpresa(
          usuarioId,
        );
      if (!datosPersonaEmpresa || datosPersonaEmpresa.length == 0)
        throw new Error('No tiene datos persona y empresa');
  ...
  async actualizarLogo(username: string, file: Express.Multer.File) {
    try {
      const usuario: any =
        await this.usuarioUsuariosRepository.findByUserName(username);
      if (!usuario) throw new Error('Usuario no existe');
      if (!usuario.persona_juridica_id)
        throw new Error('Usuario no es empresa');
      ...
      const empresaConfiguracion =
        await this.usuarioEmpresaConfiguracionRepository.findByPersonaJuridica(
          usuario.persona_juridica_id,
        );
      ...
      if (!empresaConfiguracion) {
        ...
        await this.usuarioEmpresaConfiguracionRepository.update(
          parseInt(empresaConfiguracion.empresa_configuracion_id),
          {
            logo_url: nuevoNombre,
            logo_base64:base64
          },
  ...
  async actualizarColor(color: string, username: string) {
    try {
      const usuario: any =
        await this.usuarioUsuariosRepository.findByUserName(username);
      if (!usuario) throw new Error('Usuario no existe');
